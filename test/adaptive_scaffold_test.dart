...
WidgetBuilder bodyOnly([WidgetBuilder? builder]) =>
...
void main() {
  testWidgets('adaptive scaffold lays out slots as expected', (
    ...
    expect(tester.getTopLeft(sBody), const Offset(436, 0));
    ...
    expect(tester.getTopLeft(mediumLargeSBody), const Offset(596, 0));
    ...
    expect(tester.getTopLeft(largeSBody), const Offset(696, 0));
    ...
    expect(tester.getTopLeft(extraLargeSBody), const Offset(896, 0));
    expect(tester.getTopLeft(primaryNav3), Offset.zero);
    expect(tester.getBottomRight(primaryNav3), const Offset(192, 2000));
  });

  testWidgets('adaptive scaffold animates into adaptive content layout', (
    WidgetTester tester,
  ) async {
    ...
    final Offset firstBodyPosition = tester.getTopLeft(b);
    final Offset firstSecondaryPosition = tester.getTopLeft(sBody);
    expect(firstBodyPosition.dx, greaterThan(0));
    expect(firstBodyPosition.dx, lessThan(72));
    expect(firstSecondaryPosition.dx, greaterThan(436));

    await tester.pump();
    await tester.pump(const Duration(milliseconds: 600));

    expect(tester.getTopLeft(b).dx, greaterThan(firstBodyPosition.dx));
    expect(tester.getTopLeft(b).dx, lessThan(72));
    expect(tester.getTopLeft(sBody).dx, lessThan(firstSecondaryPosition.dx));
    expect(tester.getTopLeft(sBody).dx, greaterThan(436));

    await tester.pumpAndSettle();

    expect(tester.getTopLeft(b), const Offset(72.0, 0));
    expect(tester.getBottomRight(b), const Offset(436, 2000));
    expect(tester.getTopLeft(sBody), const Offset(436, 0));
    expect(tester.getBottomRight(sBody), const Offset(800, 2000));
  });

  testWidgets(
    'adaptive content reaches final layout when animations are disabled',
  ...
  testWidgets('when destinations passed with all data, it shall not be null', (
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: MediaQuery(
          ...
          child: AdaptiveScaffold(destinations: destinations, body: bodyOnly()),
        ),
      ),
    );
    ...
    final ControllableNavRail navigationRail = tester.firstWidget(
      fNavigationRail,
    );
